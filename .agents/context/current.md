...
- Refused MCP tool calls for missing scope, read-only sections, and unavailable
  sections record metadata-only `mcp.access_denied` audit events. The audit
  record never includes profile content, prompts, bearer tokens, or secrets.
