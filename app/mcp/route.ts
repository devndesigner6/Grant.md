...
import {
  buildMcpAccessDeniedMetadata,
  classifyMcpAccessDenial,
  type McpAccessDeniedReason,
} from "@/lib/mcp-access-audit";
...
function auditMcpAccessDenied(
  ...
  after(async () => {
    await recordAuditEvent({
      userId: user.id,
      creedId: state.creedId ?? undefined,
      action: "mcp.access_denied",
      metadata: buildMcpAccessDeniedMetadata({
...
function auditSectionId(rpcRequest: JsonRpcRequest): string | undefined {
  const args = (rpcRequest.params as McpToolCallParams | undefined)?.arguments;
  return typeof args?.sectionId === "string" ? args.sectionId : undefined;
}
...
async function handleRpcRequest(
  ...
  if (rpcRequest.method === "tools/call") {
    ...
    try {
      ...
      const denialReason = classifyMcpAccessDenial(error);
      if (denialReason) {
        auditMcpAccessDenied(
          request,
          user,
